  ...
  if (code) {
    ...
    if (!error) {
      ...
      if (!isUserInDB) {
        ...
        await db
          .insert(usersTable)
          .values({ id: user!.id, name: user!.user_metadata.full_name, email: user!.email!, stripe_id: stripeID, plan: 'none' })
      }

      const forwardedHost = request.headers.get('x-forwarded-host') // original origin before load balancer
      const isLocalEnv = process.env.NODE_ENV === 'development' || forwardedHost?.startsWith('localhost')
      if (isLocalEnv) {
